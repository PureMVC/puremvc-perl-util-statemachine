package org::puremvc::perl5::multicore::utilities::statemachine::unittests::FSMInjectorTest;
...
BEGIN { use_ok( 'org::puremvc::perl5::multicore::utilities::statemachine::FSMInjector' ); }
require_ok( 'org::puremvc::perl5::multicore::utilities::statemachine::FSMInjector' );
...
sub testConstructor {
  ...
  my $fsm_injector = org::puremvc::perl5::multicore::utilities::statemachine::FSMInjector->new( { 'initial' => '' } );
  is(defined $fsm_injector, 1, 'Expecting $fsm_injector to be defined');
  isa_ok($fsm_injector, 'org::puremvc::perl5::multicore::utilities::statemachine::FSMInjector');
  ...
  $fsm_injector = org::puremvc::perl5::multicore::utilities::statemachine::FSMInjector->new( $hash );
...
sub testMethods {
  ...
  my $fsm_injector = org::puremvc::perl5::multicore::utilities::statemachine::FSMInjector->new( $hash );
...
sub testIsInitial {
  ...
  my $initial_state = org::puremvc::perl5::multicore::utilities::statemachine::State->new("initial_state");
...
sub testCreateState {
  ...
  isa_ok($state, 'org::puremvc::perl5::multicore::utilities::statemachine::State');
  is($state->{name}, "state21", 'Expecting $state->{name} to be "state21"');
  is($state->{entering}, "entering_state21", 'Expecting $state->{entering} to be "entering_state21"');
...
sub testStates {
  ...
  for (my $i = 0; $i < 3; ++$i) {
    isa_ok($states->[$i], 'org::puremvc::perl5::multicore::utilities::statemachine::State');
...
sub testInject {
  ...
  my $facade = org::puremvc::perl5::multicore::patterns::facade::Facade->getInstance("key2");
  my $hash = XMLin( $fsm_xml2, 'KeyAttr' => [], 'ForceArray' => [ 'state', 'transition' ] );
  my $fsm_injector = org::puremvc::perl5::multicore::utilities::statemachine::FSMInjector->new( $hash );
  ...
  $fsm_injector->inject();
  my $mediator = $facade->retrieveMediator( org::puremvc::perl5::multicore::utilities::statemachine::StateMachine::NAME );
  isa_ok($mediator, 'org::puremvc::perl5::multicore::utilities::statemachine::StateMachine');
